...
const Type_json_file_name = parent_json_file_name + "_type.json";
const Type_path = "./Type/" + Type_json_file_name;
const responses_type_path = "./responses_Type/" + parent_json_file_name + "_r_Type.json";
const api_responses_path = "./api_responses.json";

fs.writeFile(responses_type_path, "[]", 'utf8', (err) => {
  if (err) {
...
async function getQuestionPrompts() {
  try {
    const questions_prompts = await readFileAsync(Type_path , "utf8");
